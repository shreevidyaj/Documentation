:orphan:

====================================
Program Management and Planning
====================================

Policies
==========

*Note:  Team to identify policy(ies) as applicable*

Processes & Procedures
======================
Processes
-----------
- Release Planning Processes
   - `Process Summary <./ProgramManagement_Planning_ProcessSummary.html>`_

   - `Process Flow Diagram <../../../_static/Support/ProgramManagement_Planning/ReleasePlanning.jpg>`_

Procedures
-----------
- Sprint Management Procedure
   - `Procedure Flow Diagram <../../../_static/Support/ProgramManagement_Planning/SprintManagementProcedure.jpg>`__
   
- Sprint Analysis Procedure
   - `Procedure Flow Diagram <../../../_static/Support/ProgramManagement_Planning/StoryAnalysisProcedure.jpg>`__
   
*Note: The following are Procedures from Vx7 Jive page.  To be updated and reviewed for applicability*

- `Release Planning <https://jive.windriver.com/docs/DOC-78713>`_
- `Managing User Stories <https://jive.windriver.com/docs/DOC-61161>`_

*Note: Team to identify additional processes/procedures as applicable*

Standards & Guidelines
======================
*Note: The following are Procedures from Vx7 Jive page.  To be updated and reviewed for applicability*

- `VxWorks Release Criteria <https://jive.windriver.com/docs/DOC-69849>`_

*Note: Team to identify additional Standards & Guidelines as applicable*

Tools and Templates
===================
*Note: The following are Templates from Vx7 Jive page.  To be updated and reviewed for applicability*

- `Vx7 Release Test Strategy Template <https://jive.windriver.com/docs/DOC-50638>`_

*Note: Team to identify additional Tools & Templates as applicable*

References
==========
*Note: The following are References from Vx7 Jive page.  To be updated and reviewed for applicability*

- `Creating a Feature Test Plan <https://jive.windriver.com/docs/DOC-58077>`_

*Note: Team to identify additional References as applicable*

|
|

.. seealso::
   *In defining the Process or documents, refer to the following:*
   
   ISO 9001:2015 Section 8 - Operation
   
   `Vx7 ISO 9001 Gap Analysis <../../../../Processes/VxWorks7_ISO9001_GapAnalysis.xlsx>`_
